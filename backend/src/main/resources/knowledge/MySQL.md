...
### MySQL 字段类型 - ⭐️整数类型的 UNSIGNED 属性有什么用？
...
### MySQL 字段类型 - CHAR 和 VARCHAR 的区别是什么？
...
### MySQL 字段类型 - VARCHAR(100)和 VARCHAR(10)的区别是什么？
...
### MySQL 字段类型 - DECIMAL 和 FLOAT/DOUBLE 的区别是什么？
...
### MySQL 字段类型 - 为什么不推荐使用 TEXT 和 BLOB？
...
### MySQL 字段类型 - ⭐️DATETIME 和 TIMESTAMP 的区别是什么？如何选择？
...
### MySQL 字段类型 - ⭐️手机号存储用 INT 还是 VARCHAR？
...
### MySQL 存储引擎 - MySQL 支持哪些存储引擎？默认使用哪个？
...
### MySQL 存储引擎 - ⭐️MyISAM 和 InnoDB 有什么区别？
...
### MySQL 存储引擎 - MyISAM 和 InnoDB 如何选择？
...
### ⭐️MySQL 索引 - 为什么 InnoDB 没有使用哈希作为索引的数据结构？
...
### ⭐️MySQL 索引 - 为什么 InnoDB 没有使用 B 树作为索引的数据结构？
...
### ⭐️MySQL 索引 - 请解释一下 MySQL 的联合索引及其最左前缀原则
...
### ⭐️MySQL 性能优化 - 能用 MySQL 直接存储文件（比如图片）吗？
